...
- **epoll**：
  - 用户态和内核态的转换只发生在传递批量 fd 和 调用 recvfrom 的时候，在 fd 准备就绪的时候，内核泰会将 fd 添加到共享内存，即内核态和用户态都能访问到的空间
...
1. DNS 解析
   ...
   5. 根域名服务器向本地域名服务器返回 **gTLD 服务器**地址
...
开启 AOF 持久化后每执行一条会更改 Redis 中的数据的命令，Redis 就会**将该命令写入到内存缓存** `server.aof_buf` 中
...
- 各种方法调用。包括 Bean 自身方法，生命周期接口方法，工厂后处理器接口方法等
...
- **Around** （环绕通知）：编程式控制目标对象的方法调用。**环绕通知是所有通知类型中可操作范围最大的一种，因为它可以直接拿到目标对象，以及要执行的方法**，所以环绕通知可以任意的在目标对象的方法调用前后进行处理，甚至不调用目标对象的方
...
`EnableAutoConfiguration` 只是一个简单地注解，自动装配核心功能的实现实际是通过 `AutoConfigurationImportSelector`类.`AutoConfigurationImportSelector` 类实现了 `ImportSelector`接口，也就实现了这个接口中的 `selectImports`方法，该方法主要用于**获取所有符合条件类的全限定类名，这些类需要被加载到 IoC 容器中**。
...
- **头部(header)**：HTTP 头字段非常灵活，不仅可以使用标准里的 Host、Connection 等已有头，也可以任意添加自定义头，这就给 HTTP 协议带来了无限的扩展可能
- **实体(body)**
